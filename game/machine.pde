...
int trueMachineScreenWidth = 0; // The width of the actual screen
int trueMachineScreenHeight = 0; // The height of the actual screen
...
float machineInputScalingX = 1.0; // Value to multiply into touch input values
float machineInputScalingY = 1.0; // Value to multiply into touch input values
 ...
 *
 * This also applies the screen scaling when we're simulating a different machine's
 * screen.
 *
...
PVector inputSpaceToMachineSpace(PVector input) {
    float maxScaling = (float) Math.max(machineInputScalingX, machineInputScalingY);
    return new PVector(input.x * trueMachineScreenWidth / displayWidth * maxScaling,
                       input.y * trueMachineScreenHeight / displayHeight * maxScaling);
}
...
void initializeMachineSpecificParameters() {
    ...
    trueMachineScreenWidth = machineScreenWidth = (int) (displayWidth * scale.x);
    trueMachineScreenHeight = machineScreenHeight = (int) (displayHeight * scale.y);

    if (CONFIG_APPLY_VIRTUAL_SCALE != null) {
        machineName = CONFIG_APPLY_VIRTUAL_SCALE;
        PVector machineScreenVirtualSize = getMachineScreenVirtualSize(machineName);
        if (machineScreenVirtualSize == null) {
            System.out.printf("machine name %s does not have a specified virtual size\n", machineName);
            System.exit(1);
        }
        machineInputScalingX = machineScreenVirtualSize.x / machineScreenWidth;
        machineInputScalingY = machineScreenVirtualSize.y / machineScreenHeight;
        machineScreenWidth = (int) machineScreenVirtualSize.x;
        machineScreenHeight = (int) machineScreenVirtualSize.y;
    }
    ...
    if (machineTargetPracticeDialogPositions == null) {
        System.out.printf("machine name %s does not have specified target practice dialog positions\n", machineName);
        System.exit(1);
    }
}
...
/**
 * The regular screen size of this machine. Used to simulate a display of this
 * size when running on a different machine.
 */
private PVector getMachineScreenVirtualSize(String computerName) {
    String name = computerName.toLowerCase();
    if (name.equals("surin")) {                return new PVector(1920, 1080);
    } else if (name.equals("hci-bigscreen")) { return new PVector(1920 * 4, 1080 * 2);
    } else {                                   return null;
    }
}
